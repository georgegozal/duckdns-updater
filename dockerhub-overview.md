...
That matters because of what depends on this record. On a residential connection
it is how Let's Encrypt finds your host at renewal time, roughly every 60 days. A
stale record means the challenge reaches the wrong address and renewal fails —
and with an inline loop you find out when the certificate expires, months after
the cause.
